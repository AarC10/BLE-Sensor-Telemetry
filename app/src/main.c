...
static ssize_t recv(struct bt_conn *conn,
		    const struct bt_gatt_attr *attr, const void *buff,
...
static ssize_t recv(struct bt_conn *conn, const struct bt_gatt_attr *attr, const void *buff, uint16_t len, uint16_t offset, uint8_t flags) {
	led_update();
	return 0;
}
